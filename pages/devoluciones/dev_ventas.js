...
export default function Dev_Ventas() {
  ...
  useEffect(() => {
    ...
    const fetchDevoluciones = async () => {
      try {
        const response = await axiosInstance.get('gestion/devoluciones-con-boleta/');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-gray-900 bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-xl">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700">Boleta</label>
                <input
                  type="text"
                  value={boleta}
                  onChange={(e) => setBoleta(e.target.value.replace(/\D/g, ''))} // Reemplazar todo lo que no sea número
                  required
                  pattern="^\d+$"
  ...
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    <thead className="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
      <tr>
        ...
        <th scope="col" className="px-6 py-3">Boleta</th>
        <th scope="col" className="px-6 py-3">Acciones</th>
      </tr>
    </thead>
    <tbody>
